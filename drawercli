...
function Drawer {
    while true; 
          do 
            ...
            | fzf --prompt="$hint"                              \
            | termux-clipboard-set                              ; 
    echo "$(termux-clipboard-get)" >> $HOME/.drawercli_history
         if [ "$(termux-clipboard-get)" == "termux" ]; then 
          break
       elif [ "$(termux-clipboard-get)" == "" ] ; then
          echo "$info"; break
         fi
...
exit 0;
  ...
  if [ ! -f $HOME/.drawercli_history ]; then
      ...
      | head -n 1 \
      | fzf \
      | termux-clipboard-set ; launch $(termux-clipboard-get);
  resetClipboard;
exit ${1:+0};
  fi
